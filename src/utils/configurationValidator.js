...
export const configurationValidator = {
    ...
    validateDashboardConfig: (config) => {
      ...
      // Validate metadata
      if (config.metadata) {
        if (config.metadata.createdAt && isNaN(Date.parse(config.metadata.createdAt))) {
    ...
    validateGlobalConfig: (config) => {
      ...
      if (config.language && !validLanguages.includes(config.language)) {
        warnings.push(`Unsupported language. Recommended languages: ${validLanguages.join(', ')}`);
      }
      ...
      if (config.refreshInterval !== undefined) {
        if (typeof config.refreshInterval !== 'number') {
          ...
          warnings.push('Refresh interval is unusually high (max recommended: 1 hour)');
        }
      }
    ...
    validateEventReportConfig: (config) => {
      ...
      if (config.programType && !validProgramTypes.includes(config.programType)) {
        errors.push(`Invalid program type. Must be one of: ${validProgramTypes.join(', ')}`);
      }
